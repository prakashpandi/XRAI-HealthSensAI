...
public class DemoIntegration : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            if (Keyboard.current[pushToTalkKey].wasPressedThisFrame && !isRecording) StartRecording();
            if (Keyboard.current[pushToTalkKey].wasPressedThisFrame && isRecording) StopRecording();
        }
        UpdateFrequencyBars();
